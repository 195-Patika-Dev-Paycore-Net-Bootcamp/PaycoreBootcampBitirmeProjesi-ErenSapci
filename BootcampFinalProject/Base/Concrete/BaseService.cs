...
namespace BootcampFinalProject.Base.Concrete
{
    public abstract class BaseService<Dto, Entity> : IBaseService<Dto, Entity> where Entity : class
    {
        ...
        public virtual BaseResponse<Dto> Update(int id, Entity updateResource)
        {
            try
            {
                var tempEntity = hibernateRepository.GetById(id);
                ...
                var resource = mapper.Map<Entity, Dto>(updateResource);
                return new BaseResponse<Dto>(resource);
            }
            catch (Exception ex)
            {
